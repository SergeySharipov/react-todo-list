...
function Category({ category, selectCategory, editable = true, selectedCategory, removeCategory }) {
  return (
    <div className={selectedCategory.id === category.id ? 'Category-selected' : 'Category'} onClick={selectCategory}>
      ...
      {editable && <div className='Category-actions'>
        ...
        <button className="Category-deleteButton"><img src={ic_delete} alt="Delete" onClick={removeCategory} /></button>
      </div>}
    </div>
  );
}
